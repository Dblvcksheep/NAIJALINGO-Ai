...
def add_language(request: Request,name: str = Form(...), region: str = Form(...), family: str = Form(...), db: Session = Depends(get_db)):
    user = request.session.get("user")
...
def submit_review(request: Request,applicant_id: int = Form(...), decision: str = Form(...), db: Session = Depends(get_db)):
    user = request.session.get("user")
